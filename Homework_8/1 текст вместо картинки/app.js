...
function handleClick(clickedButtonEvent) {
    ...
    const productImg = cardNode.querySelector('img');
    const productName = cardNode.querySelector('.productName');
    const productButton = cardNode.querySelector('button');
    const card = {
        wrap: cardNode, // здесь элемент с классом .product
        img: productImg, // здесь картинка внутри .product
        productName: productName, // здесь .productName, который внутри .product
        button: productButton, // здесь button, который внутри .product
    };

    // 4. получаем текст на кнопке, которая внутри .product
    let textButton = clickedButtonEvent.target.innerHTML;
...
function hideMoreText(card) {
    ...
    let element = card.wrap.querySelector('.desc');
    element.remove();
...
function showMoreText(card) {
    ...
    let textCard = "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.";
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    card.productName.insertAdjacentHTML('afterend', `<div class="desc">${textCard}</div>`)
